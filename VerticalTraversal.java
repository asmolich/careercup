public class VerticalTraversal {
    public ArrayList<ArrayList<Integer>> verticalOrderTraversal(TreeNode head) {
        if (head == null) return new ArrayList<ArrayList<Integer>>();
        ...
        while (!q.isEmpty()) {
            ...
            ArrayList<Integer> currentLevel = res.getOrDefault(nodeLevel.level, new ArrayList<Integer>());
...
class TreeNodeLevel {
    ...
    static TreeNodeLevel of(TreeNode node, int level) {
        ...
        nodeLevel.node = node; nodeLevel.level = level;
        return nodeLevel;
    }
}
